...
export function loadTradesFromCSV(): Trade[] {
  ...
  const tradeExecutions = executions.filter(
    e => e.execType === 'Trade' && e.side && e.lastQty > 0 && e.orderID
  );
...
export function calculateTradingStats(): TradingStats {
  ...
  const realizedPnlTxs = wallet.filter(
    w => w.transactType === 'RealisedPNL' && w.transactStatus === 'Completed'
  );
  const fundingTxs = wallet.filter(
    w => w.transactType === 'Funding' && w.transactStatus === 'Completed'
  );
  ...
  const fundingPaid =
    fundingTxs.filter(w => w.amount < 0).reduce((sum, w) => sum + Math.abs(w.amount), 0) /
    SAT_TO_BTC;
  const fundingReceived =
    fundingTxs.filter(w => w.amount > 0).reduce((sum, w) => sum + w.amount, 0) / SAT_TO_BTC;
  ...
  return {
    ...
    winRate:
      winningTxs.length + losingTxs.length > 0
...
export function getPaginatedTrades(
  ...
  if (symbol) {
    ...
    filtered = allTrades.filter(
      t => t.symbol === symbol || t.symbol === internalSymbol || t.displaySymbol === symbol
    );
  }
