#!/usr/bin/env bun
/**
 * Advanced Bun YAML Features Comprehensive Demonstration
 * 
 * This example showcases the full power of Bun v1.3's YAML support, including
 * advanced features that make YAML a first-class citizen in the Bun ecosystem.
 * 
...
console.log('🚀 Advanced Bun YAML Features - Comprehensive Demo');
console.log('==================================================');
...
try {
  ...
  docs.forEach((doc, index) => {
    ...
    // Display document-specific properties
  ...
  const configDocs = docs.filter(doc => doc.type === 'config');
  const settingsDocs = docs.filter(doc => doc.type === 'settings');
  const monitoringDocs = docs.filter(doc => doc.type === 'monitoring');
  
  console.log(`\n🔍 Document Analysis:`);
  console.log(`   • Config documents: ${configDocs.length}`);
  console.log(`   • Settings documents: ${settingsDocs.length}`);
  console.log(`   • Monitoring documents: ${monitoringDocs.length}`);
...
// ═══════════════════════════════════════════════════════════════
// ADVANCED YAML SYNTAX DEMONSTRATION
// ═══════════════════════════════════════════════════════════════

console.log('\n🎨 2️⃣ Advanced YAML Syntax Features:');
console.log('------------------------------------');
...
# Explicit type tags demonstration
config: !!str "123"
port: !!int 8080
timeout: !!float 30.5
enabled: !!bool true
...
try {
  ...
  // Debug: Show actual parsed structure
  console.log('\n🔍 Debug - Parsed Structure:');
  console.log('   Available keys:', Object.keys(advancedData));
  console.log('   Description type:', typeof advancedData.description);
  console.log('   Description value:', advancedData.description);
  console.log('   Summary type:', typeof advancedData.summary);
  console.log('   Summary value:', advancedData.summary);
  ...
  if (advancedData.description) {
    console.log(`   "${advancedData.description.trim().split('\n').join(' ')}"`);
  } else {
    console.log('   ⚠️  Description field is undefined or null');
  }
  console.log('\n   Folded (>):');
  if (advancedData.summary) {
    console.log(`   "${advancedData.summary}"`);
  } else {
    console.log('   ⚠️  Summary field is undefined or null');
  }
  ...
  if (typeof obj === 'string') {
    ...
    return obj.replace(/\$\{([^}]+)\}/g, (match, varSpec) => {
      ...
      if (envValue !== undefined) {
        ...
        if (context.includes('port') || context.includes('timeout')) {
          const num = parseInt(envValue, 10);
          return isNaN(num) ? envValue : num;
        }
        if (context.includes('enabled') || context.includes('debug')) {
...
process.env.TEST_SERVER_PORT = '9000';
process.env.TEST_DB_HOST = 'prod.db.example.com';
process.env.TEST_DEBUG_MODE = 'false';
process.env.TEST_API_KEY = 'sk-production-123456789';

const envConfig = {
  server: {
    port: '${TEST_SERVER_PORT:-3000}',
    host: '${TEST_DB_HOST:-localhost}',
    debug: '${TEST_DEBUG_MODE:-true}'
  },
  database: {
    connection_string: 'postgresql://${TEST_DB_HOST:-localhost}:5432/trader_analyzer',
    ssl: '${TEST_SSL_MODE:-require}',
    pool_size: '${TEST_POOL_SIZE:-10}'
  },
  api: {
    key: '${TEST_API_KEY}',
...
delete process.env.TEST_DEBUG_MODE;
delete process.env.TEST_API_KEY;

// ═══════════════════════════════════════════════════════════════
// NAMED IMPORTS FROM ENHANCED CONFIG
// ═══════════════════════════════════════════════════════════════

console.log('\n📦 4️⃣ Named Imports from Enhanced Config:');
console.log('--------------------------------------------');

try {
  // Import specific sections from our enhanced configuration
  const enhancedConfig = await import('../config/enhanced-config-single.yaml');
  const { development, staging, production } = enhancedConfig;
  ...
  const environments = [
    { name: 'Development', config: development },
    { name: 'Staging', config: staging },
    { name: 'Production', config: production }
  ];
  
  environments.forEach(({ name, config }) => {
    console.log(`\n   📋 ${name}:`);
    console.log(`      Server Port: ${config.server.port}`);
    console.log(`      Database Host: ${config.database.host}`);
    console.log(`      Debug Mode: ${config.server.debug}`);
    console.log(`      Max Topics: ${config.threadManager.maxTopicsPerChat}`);
  });
  
  // Check if monitoring and logging exist before accessing
  if (enhancedConfig.monitoring && enhancedConfig.logging) {
    const { monitoring, logging } = enhancedConfig;
    
    // Display monitoring configuration
    console.log('\n📊 Monitoring Configuration:');
    console.log(`   Enabled: ${monitoring.enabled}`);
    console.log(`   Metrics Port: ${monitoring.metricsPort}`);
    console.log(`   Health Check: ${monitoring.healthCheck.enabled}`);
    console.log(`   Alert Endpoints: ${monitoring.alerting.endpoints.length}`);
    
    // Display logging configuration
    console.log('\n📝 Logging Configuration:');
    console.log(`   Level: ${logging.level}`);
    console.log(`   Format: ${logging.format}`);
    console.log(`   Console Output: ${logging.console.enabled}`);
    console.log(`   File Output: ${logging.file.enabled}`);
  } else {
    console.log('\n⚠️  Monitoring and logging configurations not available');
  }
...
console.log('\n🚦 6️⃣ Feature Flags Configuration System:');
console.log('-----------------------------------------');
...
try {
  const featuresData = YAML.parse(featureFlagsYaml) as { features: FeatureFlags };
  ...
  /**
   * Advanced feature flag evaluation with context
   */
...
console.log('\n🏭 7️⃣ Production Patterns & Best Practices:');
console.log('-------------------------------------------');
...
console.log('\n🎯 Advanced YAML Features Demo Summary:');
console.log('======================================');
...
console.log('✅ Named imports and destructuring verified');
