...
const styleLabels: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
    scalper: { label: 'Scalper', icon: <Zap className="w-4 h-4" />, color: 'text-violet-300 bg-violet-500/20 border border-violet-500/30' },
    day_trader: { label: 'Day Trader', icon: <Activity className="w-4 h-4" />, color: 'text-sky-300 bg-sky-500/20 border border-sky-500/30' },
    swing_trader: { label: 'Swing Trader', icon: <TrendingUp className="w-4 h-4" />, color: 'text-teal-300 bg-teal-500/20 border border-teal-500/30' },
    position_trader: { label: 'Position Trader', icon: <Timer className="w-4 h-4" />, color: 'text-emerald-300 bg-emerald-500/20 border border-emerald-500/30' }
};

const riskLabels: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
    aggressive: { label: 'Aggressive', icon: <Flame className="w-4 h-4" />, color: 'text-rose-300 bg-rose-500/20 border border-rose-500/30' },
    moderate: { label: 'Moderate', icon: <Shield className="w-4 h-4" />, color: 'text-amber-300 bg-amber-500/20 border border-amber-500/30' },
    conservative: { label: 'Conservative', icon: <Snowflake className="w-4 h-4" />, color: 'text-sky-300 bg-sky-500/20 border border-sky-500/30' }
};
...
const demoData: TraderProfileData = {
    ...
    profile: {
        summary: 'Moderate swing trader, 58% win rate, 1.85 profit factor, strong discipline, avg holding time ~18.5 hours.',
        strengths: [
            'High Win Rate (58.32%)',
        ...
        weaknesses: [
            'Tendency to Close Early (Score: 68/100)',
        ...
        <div className="space-y-5">
            ...
            {!isDemoMode && currentData.raw && (
                <div className="flex items-center gap-3 px-4 py-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                    ...
                    <span className="text-emerald-300 text-sm font-medium">Real Data Mode - Based on {currentData.raw.basic_stats?.total_orders?.toLocaleString() || 0} orders analyzed</span>
            ...
            {isDemoMode && (
                <div className="flex items-center gap-3 px-4 py-3 bg-sky-500/10 border border-sky-500/30 rounded-xl">
                    ...
                    <span className="text-sky-300 text-sm font-medium">Demo Mode - Connect API to see real data</span>
                </div>
            )}
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50">
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                        <div className="text-xs uppercase tracking-wider text-slate-500 mb-2 font-semibold">Style Summary</div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
                        <div className="flex items-center gap-2 text-slate-400 mb-2">
                            ...
                            <span className="text-xs font-medium uppercase tracking-wider">Limit Order Ratio</span>
                        </div>
