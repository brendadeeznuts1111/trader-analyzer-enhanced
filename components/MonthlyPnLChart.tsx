  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-start mb-4">
        <div>
          <p className="text-sm text-muted-foreground">{safeData.length} months tracked</p>
        </div>
        <div className="text-right">
          <p
            className={`text-xl font-bold ${totalPnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}
          >
            {totalPnl >= 0 ? '+' : ''}
            {totalPnl.toFixed(4)} BTC
          </p>
          <p className="text-xs text-muted-foreground">
            {profitableMonths}/{safeData.length} profitable months (
            {((profitableMonths / safeData.length) * 100).toFixed(0)}%)
          </p>
        </div>
      </div>
            ...
            {[...safeData]
              ...
              .map(d => (
                <tr key={d.month} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td
                    className={`py-2 text-right font-bold ${d.pnl >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}
                  >
                    {d.pnl >= 0 ? '+' : ''}
                    {d.pnl.toFixed(4)}
                  </td>
                  <td
                    className={`py-2 text-right ${d.funding >= 0 ? 'text-emerald-500' : 'text-amber-500'}`}
                  >
                    {d.funding >= 0 ? '+' : ''}
                    {d.funding.toFixed(4)}
                  </td>
