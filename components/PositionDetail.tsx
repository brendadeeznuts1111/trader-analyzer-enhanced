...
export function PositionDetail({ session, onBack }: PositionDetailProps) {
  ...
  const pnlPercent =
    session.avgEntryPrice > 0 && session.avgExitPrice > 0
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight flex items-center gap-3">
            ...
            <span
              className={`text-sm font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${
                session.side === 'long'
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div
          className={`glass rounded-2xl p-1 ${
            isProfit ? 'ring-1 ring-emerald-500/20' : 'ring-1 ring-rose-500/20'
          }`}
        >
          <div
            className={`bg-card/50 rounded-xl p-5 h-full ${
          ...
          >
            ...
            <div
              className={`text-2xl font-bold tracking-tight ${isProfit ? 'text-emerald-500' : 'text-rose-500'}`}
            >
              {isProfit ? '+' : ''}
              {session.netPnl.toFixed(6)} <span className="text-lg opacity-70">XBT</span>
            </div>
            {session.status === 'closed' && (
              <div
                className={`text-sm font-medium mt-1 ${isProfit ? 'text-emerald-500' : 'text-rose-500'}`}
              >
                {pnlPercent >= 0 ? '+' : ''}
        ...
        <div className="glass rounded-2xl p-1">
          <div className="bg-card/50 rounded-xl p-5 h-full">
            ...
            <div className="text-xl font-bold">{session.maxSize.toLocaleString()}</div>
            <div className="text-sm text-muted-foreground font-medium mt-1">
          ...
          {tradesWithPosition.map((trade, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center ring-4 ring-background ${
                      trade.side === 'buy'
                ...
                <div className="text-right">
                  ...
                  <div
                    className={`text-xs font-medium ${
                      trade.positionAfter - trade.positionBefore > 0
                        ? 'text-emerald-500'
                        : 'text-rose-500'
                    }`}
                  >
                    {trade.positionBefore >= 0 ? '+' : ''}
                    {trade.positionAfter - trade.positionBefore}
                  </div>
                </div>
              </div>
            </div>
