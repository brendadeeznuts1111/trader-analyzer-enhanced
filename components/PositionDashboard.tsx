...
export function PositionDashboard({
  ...
  // Generate chart markers from selected session trades
  const chartMarkers = useMemo(() => {
    if (!selectedSession || !selectedSession.trades || chartData.candles.length === 0) {
      return [];
    }
  ...
  }, [selectedSession, timeframe, chartData.candles]);

  // Handler to select a session
  const handleSelectSession = async (session: PositionSession) => {
    try {
      const response = await fetch(`/api/trades?sessionId=${encodeURIComponent(session.id)}`);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section>
        {selectedSession ? (
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold tracking-tight text-foreground">Position History</h2>
            ...
            <PositionSessionList sessions={sessions} onSelectSession={handleSelectSession} />
          </>
        )}
      </section>
    </div>
